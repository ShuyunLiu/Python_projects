...
def add_file(name_data, filename):
    ...
    with open(filename, 'r') as f:
        for line in f:
            info_lst = line.split(',')              # every vocab is seperated by ,
            if len(info_lst) == 1:
                year = info_lst[0].strip()
            else:
                rank = info_lst[0].strip()
                name_boy = info_lst[1].strip()      # boy name
                name_girl = info_lst[2].strip()      # girl name
...
def read_files(filenames):
    ...
    """
    name_data = {}                  # name_data is a dictionary, will return at the end
    for file in filenames:          # read every file(name)
        add_file(name_data, file)   # add all filename to name_data
    return name_data
...
def search_names(name_data, target):
    ...
    """
    result = []                             # empty list, will store name later
    for name in name_data:
        if target.lower() in name.lower():  # case-insensitive
            result.append(name)             # add in the list
    return result
